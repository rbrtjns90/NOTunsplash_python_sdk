...
def test_user_model():
    ...
    assert user.portfolio_url == user_data["portfolio_url"]
...
def test_photo_model():
    ...
    photo = Photo.from_dict(MOCK_PHOTO_RESPONSE)
    ...
    assert photo.description == MOCK_PHOTO_RESPONSE["description"]
    ...
    assert photo.user.username == MOCK_PHOTO_RESPONSE["user"]["username"]

def test_collection_model():
    """Test Collection model creation and properties"""
    collection = Collection.from_dict(MOCK_COLLECTION_RESPONSE)
    
    assert collection.id == MOCK_COLLECTION_RESPONSE["id"]
    assert collection.title == MOCK_COLLECTION_RESPONSE["title"]
    assert collection.description == MOCK_COLLECTION_RESPONSE["description"]
    ...
    assert collection.user.username == MOCK_COLLECTION_RESPONSE["user"]["username"]

def test_topic_model():
    """Test Topic model creation and properties"""
    topic = Topic.from_dict(MOCK_TOPIC_RESPONSE)
    ...
    assert topic.description == MOCK_TOPIC_RESPONSE["description"]
